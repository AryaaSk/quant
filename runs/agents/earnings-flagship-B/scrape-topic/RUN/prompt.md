...
- Topic for this invocation: **RUN**
- Search queries to start from: **["RUN earnings preview 2020 Qq", "RUN earnings preview 2020 Qq", "RUN earnings preview 2021 Qq", "RUN earnings preview 2021 Qq", "RUN earnings preview 2021 Qq", "RUN earnings preview 2021 Qq", "RUN earnings preview 2022 Qq", "RUN earnings preview 2022 Qq", "RUN earnings preview 2022 Qq", "RUN earnings preview 2022 Qq", "RUN earnings preview 2023 Qq", "RUN earnings preview 2023 Qq", "RUN earnings preview 2023 Qq", "RUN earnings preview 2023 Qq", "RUN earnings preview 2024 Qq", "RUN earnings preview 2024 Qq", "RUN earnings preview 2024 Qq", "RUN stock news preview", "RUN guidance outlook quarterly results", "RUN analyst expectations", "RUN retail investor sentiment"]**
- Target article count: **30** (quality over quantity)
- Output directory (absolute): **/Users/aryaask/Desktop/quant/data/raw/earnings-flagship-B/_news/RUN**
...
3. Save each article as a separate JSON file in `/Users/aryaask/Desktop/quant/data/raw/earnings-flagship-B/_news/RUN/` named `article_<NNNN>.json`.
4. Aim for **30** quality articles total across all queries. Stop early if budget runs low.
5. After saving, write a manifest file `/Users/aryaask/Desktop/quant/data/raw/earnings-flagship-B/_news/RUN/_done` containing a single line: `count=<N> at=<ISO8601_now>`.
...
- Do not write code execution / Bash beyond `mkdir -p /Users/aryaask/Desktop/quant/data/raw/earnings-flagship-B/_news/RUN` and simple file ops.
- Do not invent timestamps. Use `null` for `published_at` when unknown.
- Do not save articles with empty or junk `text`.
- Do not write any file outside `/Users/aryaask/Desktop/quant/data/raw/earnings-flagship-B/_news/RUN`.
