...
- Topic for this invocation: **injuries**
- Search queries to start from: **["English League One injury list ruled out", "EFL League Two player injury hamstring", "Championship loan move January window"]**
- Target article count: **80** (quality over quantity)
- Output directory (absolute): **/Users/aryaask/Desktop/quant/data/raw/betfair-eng-l1l2/_news/injuries**
...
3. Save each article as a separate JSON file in `/Users/aryaask/Desktop/quant/data/raw/betfair-eng-l1l2/_news/injuries/` named `article_<NNNN>.json`.
4. Aim for **80** quality articles total across all queries. Stop early if budget runs low.
5. After saving, write a manifest file `/Users/aryaask/Desktop/quant/data/raw/betfair-eng-l1l2/_news/injuries/_done` containing a single line: `count=<N> at=<ISO8601_now>`.
...
```json
{
  ...
  "topic": "injuries",
  "source_type": "news_scraper.injuries"
}
```
...
- Do not write code execution / Bash beyond `mkdir -p /Users/aryaask/Desktop/quant/data/raw/betfair-eng-l1l2/_news/injuries` and simple file ops.
- Do not invent timestamps. Use `null` for `published_at` when unknown.
- Do not save articles with empty or junk `text`.
- Do not write any file outside `/Users/aryaask/Desktop/quant/data/raw/betfair-eng-l1l2/_news/injuries`.
